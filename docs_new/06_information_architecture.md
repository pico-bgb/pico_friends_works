...
- **신뢰성 확보**: 사진 인증 시 현장 촬영만 허용하고 위치 정보를 함께 기록하여 활동 데이터의 신뢰도 확보

### 1.2 주요 사용자

| 역할 | 주요 과업 |
|------|----------|
| **피코프렌즈** | 배정된 약국 목록 확인, 약국 방문 후 사진/위치 인증, 설문 조사 제출, 현장 피드백 제출, 본인 활동 내역 조회 |
...
| PF-031 | | (Step 1) | 방문 인증 (사진/위치) | - 화면 진입 시 즉시 디바이스의 카메라 앱 실행<br>- 약국 전경 촬영 후 '확인' 버튼으로 업로드<br>- 업로드 시 사진 정보가 함께 서버로 전송 | - 갤러리 접근 절대 불가 | 신뢰도 확보 핵심 기능 |
...
| **방문 인증 (사진/위치)** | - 약국 선택 시, 사진 촬영 화면으로 이동<br>- 디바이스의 카메라를 직접 실행하여 촬영 후 업로드<br>- 신뢰도 확보를 위해 갤러리 접근 차단 |
...
| **데이터 조회 및 추출** | - 제출된 모든 활동 데이터(사진, 위치, 설문 답변, 피드백)를 조건별로 검색 및 필터링<br>- 조회된 모든 데이터를 엑셀(CSV) 파일로 다운로드하는 기능 필수 |
